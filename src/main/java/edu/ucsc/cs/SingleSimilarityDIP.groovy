...
class SingleSimilarityDIP {
    ...
    def setupDataStore(config) {
    	String dbpath = "./psl_fsbio_singleSimilarityDIP";
    ...
    def defineModel(config, data, m){
    	...
    	m.add rule : (Similar(D1, D2) & interacts(D1, D3) & validInteraction(D1, D3) & validInteraction(D2, D3) & (D2 - D3) & (D1 - D2))>>interacts(D2, D3) , weight:config.initialWeight
		...
		config.closedPredicatesInference = [validInteraction, Similar];
	...
	def loadData(data,config, predString) { 
	    ...
	    log.debug("Reading " + predString + " ...");
    	insert = data.getInserter(Similar, readSimilarities)
    	InserterUtils.loadDelimitedDataTruth(insert, simDir + predString.toLowerCase() + ".csv")
	...
	def loadSimilarityData(config, data, df, pred){
		def cvfold = df.cvFold;
		def wlfold = df.wlFold;
		def predName = pred.getName().toLowerCase();
		...
		df.wlSim = data.getPartition("wlsim_" + predName + wlfold)
		df.cvSim = data.getPartition("cvsim_" + predName + cvfold)

		log.debug("Reading " + p.getName());

		def insert = data.getInserter(pred, df.cvSim)
		InserterUtils.loadDelimitedDataTruth(insert, current_sim_dir + predName + "_cv.csv")
		insert = data.getInserter(p, df.wlSim)
		InserterUtils.loadDelimitedDataTruth(insert, current_sim_dir + predName + "_wl.csv")
  ...
  // =========
	...
	def evaluateResults(data,config,df,de, truthPartition, predictionPartition, writeToFile){ 
	  ...
	  def runExperiment(numDrugs, numFolds, experimentName, blockingNum, blockingType, interactionType, createDS, predString){
	    ...
	    //def pf = edu.umd.cs.psl.model.predicate.PredicateFactory.getFactory();
		//def pred = pf.getPredicate(predString);

	    if(config.createNewDataStore){
			this.loadData(data, config, predString);
		}
	  ...
	  static void main(args){
	  	...
	  	for(String exp: experiments){
	  		...
	  		for(String it: interactionTypes){
	  			...
	  			for (String predString : ["ATCSimilarity", "distSimilarity", "seqSimilarity", "ligandSimilarity", "GOSimilarity", "SideEffectSimilarity", "chemicalSimilarity"]){
	  				System.out.println("Working on interaction type " + it + " with blocking type " + blockingType + " and similarity " + predString);
		  			def dip = new SingleSimilarityDIP();
		  			def evaluation = dip.runExperiment(numDrugs, 10, exp, blockingNum, blockingType, it, true, predString);
		  			
		  			dip.outputResultsCSV(it, exp + "_" + predString, evaluation.AUC_Folds, "auc");
		  			dip.outputResultsCSV(it, exp + "_" + predString, evaluation.AUPR_N_Folds, "auprNeg");
		  			dip.outputResultsCSV(it, exp + "_" + predString, evaluation.AUPR_P_Folds, "auprPos");
