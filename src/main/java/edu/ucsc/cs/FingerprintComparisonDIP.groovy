...
class FingerprintComparisonDIP {
	...
	def loadData(data,config,fingerprintType) { 
	    ...
	    for (Predicate p : [ATCSimilarity, distSimilarity, seqSimilarity, ligandSimilarity, GOSimilarity, SideEffectSimilarity])
	    {
	    ...
	    log.debug("Reading chemical similarity...");
    	insert = data.getInserter(chemicalSimilarity, readSimilarities)
    	InserterUtils.loadDelimitedDataTruth(insert, simDir + fingerprintType + "_chemicalSimilarity.csv")
	}
  ...
  // =========
	...
	def evaluateResults(data,config,df,de, truthPartition, predictionPartition, writeToFile){ 
	  ...
	  def runExperiment(numDrugs, numFolds, experimentName, blockingNum, blockingType, interactionType, createDS, fingerprintType){
	    ...
	    if(config.createNewDataStore){
			this.loadData(data, config, fingerprintType);
		}
	  ...
	  static void main(args){
	  	...
	  	String exp = 'all_dataset2';
	  	String it = 'all';
	  	def numDrugs = 315;
	  	def blockingNum = 15;
	  	def blockingType = 5;

	  	for (String fingerprintType : ["daylight", "maccs", "morgan", "morganFeatureBased", "atomPair"]){
			System.out.println("Working on interaction type " + it + " with blocking type " + blockingType + " and fingerprint type " + fingerprintType);
			def dip = new FingerprintComparisonDIP();
			def evaluation = dip.runExperiment(numDrugs, 10, exp, blockingNum, blockingType, it, true, fingerprintType);
			
			dip.outputResultsCSV(it, exp + "_" + fingerprintType, evaluation.AUC_Folds, "auc");
			dip.outputResultsCSV(it, exp + "_" + fingerprintType, evaluation.AUPR_N_Folds, "auprNeg");
			dip.outputResultsCSV(it, exp + "_" + fingerprintType, evaluation.AUPR_P_Folds, "auprPos");
		}
  	}
}