...
class NonCollectivePrediction {
    ...
    def setupDataStore(config) {
    	String dbpath = "/scratch0/psl_db_fsbio_noncollective";
	...
	def defineModel(config, data, m){
    	...
    	m.add rule : (ATCSimilarity(D1, D2) & observedInteracts(D2, D3) & validInteraction(D1, D3) & (D2 - D3) & (D1 - D2))>>interacts(D1, D3) , weight:config.initialWeight
		m.add rule : (SideEffectSimilarity(D1, D2) & observedInteracts(D2, D3) & validInteraction(D1, D3) & (D2 - D3) & (D1 - D2))>>interacts(D1, D3) , weight:config.initialWeight
		m.add rule : (GOSimilarity(D1, D2) & observedInteracts(D2, D3) & validInteraction(D1, D3) & (D2 - D3) & (D1 - D2))>>interacts(D1, D3) , weight:config.initialWeight
		m.add rule : (ligandSimilarity(D1, D2) & observedInteracts(D2, D3) & validInteraction(D1, D3) & (D2 - D3) & (D1 - D2))>>interacts(D1, D3) , weight:config.initialWeight
		m.add rule : (chemicalSimilarity(D1, D2) & observedInteracts(D2, D3) & validInteraction(D1, D3) & (D2 - D3) & (D1 - D2))>>interacts(D1, D3) , weight:config.initialWeight
		m.add rule : (seqSimilarity(D1, D2) & observedInteracts(D2, D3) & validInteraction(D1, D3) & (D2 - D3) & (D1 - D2))>>interacts(D1, D3) , weight:config.initialWeight
		m.add rule : (distSimilarity(D1, D2) & observedInteracts(D2, D3) & validInteraction(D1, D3) & (D2 - D3) & (D1 - D2))>>interacts(D1, D3) , weight:config.initialWeight
	...
	def loadInteractionsData(config, data, df, isSupervisedMode){
		...
		def insertWLObservedInteracts = data.getInserter(observedInteracts, df.wlTrain);

		def insertCVValid = data.getInserter(validInteraction, df.cvTrain);
		def insertCVLabels = data.getInserter(interacts, df.cvTruth);
		def insertCVObservedInteracts = data.getInserter(observedInteracts, df.cvTrain);
		...
		for (int j=1;j<=config.numFolds;j++)
		{
			...
			if ((j!=cvfold) && (j!=wlfold))
			{
				...
				if(isSupervisedMode){
					...
					InserterUtils.loadDelimitedDataTruth(insertWLObservedInteracts, current_interactions_dir + interactions_file);
					InserterUtils.loadDelimitedDataTruth(insertCVObservedInteracts, current_interactions_dir + interactions_file);

				}else{

					InserterUtils.loadDelimitedDataTruth(insertWLObservedInteracts, current_interactions_dir + interactions_positive);
					InserterUtils.loadDelimitedDataTruth(insertCVObservedInteracts, current_interactions_dir + interactions_positive);
		...
		if(isSupervisedMode){
			...
			InserterUtils.loadDelimitedDataTruth(insertCVObservedInteracts, train_interactions_dir + interactions_file);
			...
			InserterUtils.loadDelimitedDataTruth(insertCVObservedInteracts, train_interactions_dir + interactions_positive);
  ...
  // =========
	...
	def evaluateResults(data,config,df,de, truthPartition, predictionPartition, writeToFile){ 
	    ...
	    if(writeToFile){
	    	OutputWriter writer = new OutputWriter(predictionsDB, interacts, 'noncollective_' + config.interaction_type, "allFolds", "psl");
	  ...
	  static void main(args){
	  	...
	  	//String[] experiments = ['dataset2_nonsymmetric_test']
	  	def interactionTypes = ['all'];
	  	...
	  	for(String exp: experiments){
	  		...
	  		for(String it: interactionTypes){
	  			...
	  			def dip = new NonCollectivePrediction();
	  			def evaluation = dip.runExperiment(numDrugs, 10, exp, blockingNum, blockingType, it, true);

	  			dip.outputResultsCSV(it, exp, evaluation.AUC_Folds, "auc_nonCollective");
	  			dip.outputResultsCSV(it, exp, evaluation.AUPR_N_Folds, "auprNeg_nonCollective");
	  			dip.outputResultsCSV(it, exp, evaluation.AUPR_P_Folds, "auprPos_nonCollective");
